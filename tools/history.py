...
def _classify_issue(current: Optional[dict], previous: Optional[dict]) -> tuple[str, float]:
    if current and not previous:
        return "new", float(current.get("priority_score", 0.0) or 0.0)
    if previous and not current:
        return "disappeared", -float(previous.get("priority_score", 0.0) or 0.0)
    ...
    if delta_priority >= 5.0 or delta_evidence > 0 or delta_independent > 0:
        return "rising", delta_priority
    if delta_priority <= -5.0 or delta_evidence < 0 or delta_independent < 0:
        return "declining", delta_priority
    return "stable", delta_priority


def _issue_diff_row(issue_id: str, current: Optional[dict], previous: Optional[dict]) -> dict:
    status_label, delta_vs_prev = _classify_issue(current, previous)
    ...
    if emit_diff_report:
        ...
        lines.extend(
            [
                ...
                "## Issue Changes",
                "",
                "| Issue | Status | Delta priority | Evidence now | Evidence prev | Independent now | Independent prev |",
                "|------|--------|---------------:|-------------:|--------------:|----------------:|-----------------:|",
            ]
        )
        for issue in history_data.get("issues", []):
            lines.append(
                f"| {issue['canonical_issue_id']} | {issue['status_label']} | {issue['delta_vs_prev']:.2f} | "
